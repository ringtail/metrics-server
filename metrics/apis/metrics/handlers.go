...
func (a *Api) nodeMetricsList(request *restful.Request, response *restful.Response) {
	...
	nodes, err := a.nodeLister.ListWithPredicate(func(node *v1.Node) bool {
...
func (a *Api) nodeMetrics(request *restful.Request, response *restful.Response) {
	...
	response.WriteEntity(NodeMetrics(*m))
}

func (a *Api) getNodeMetrics(node string) *NodeMetrics {
...
func parseResourceList(ms *core.MetricSet) (metrics.ResourceList, error) {
	...
	return metrics.ResourceList{
		metrics.ResourceName(v1.ResourceCPU): *resource.NewMilliQuantity(
			cpu.IntValue,
			resource.DecimalSI),
		metrics.ResourceName(v1.ResourceMemory): *resource.NewQuantity(
			mem.IntValue,
			resource.BinarySI),
	}, nil
}
...
func (a *Api) allPodMetricsList(request *restful.Request, response *restful.Response) {
	podMetricsInNamespaceList(a, request, response, v1.NamespaceAll)
}
...
func podMetricsInNamespaceList(a *Api, request *restful.Request, response *restful.Response, namespace string) {
	...
	for _, pod := range pods {
		if m := a.getPodMetrics(pod); m != nil {
			res.Items = append(res.Items, PodMetrics(*m))
		} else {
...
func (a *Api) getPodMetrics(pod *v1.Pod) *PodMetrics {
	batch := a.metricSink.GetLatestDataBatch()
	if batch == nil {
		return nil
	}
	...
	for _, c := range pod.Spec.Containers {
		...
		res.Containers = append(res.Containers, ContainerMetrics{Name: c.Name, Usage: usage})
	}
	return res
}
