...
const (
	...
	// used for kubetunnel
	kubeTunnelServerInsecurePort   = 10264
...
func (this *summaryMetricsSource) ScrapeMetrics(start, end time.Time) *DataBatch {
	...
	summary, err := func() (*stats.Summary, error) {
		...
		glog.Infof("scrape node(%s) summary metrics from host(%s)", this.node.NodeName, this.node.Host.String())
		return this.kubeletClient.GetSummary(this.node.Host, this.node.NodeName)
	}()
...
func (this *summaryProvider) getNodeInfo(node *corev1.Node) (NodeInfo, error) {
	...
	if enableTunnel, ok := node.Labels[labelEnableReverseTunnelClient]; ok && enableTunnel == "true" {
		info.Host.Port = kubeTunnelServerInsecurePort
		info.IP = "127.0.0.1"
	} else {
...
func NewKubeTunnelProvider(uri *url.URL) (MetricsSourceProvider, error) {
	// create clients
